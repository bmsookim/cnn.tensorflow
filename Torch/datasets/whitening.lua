...
function zca_whiten(data, means, P, invP, epsilon)
...
function zca_colour(data, means, P, invP)
    local auxdata = data:clone()
    local dims = data:size()
    local nsamples = dims[1]
    local n_dimensions = data:nElement() / nsamples
...
function zca_layer(data, means, P, invP)
    local auxdata
    if not means or not P or not invP then 
        auxdata, means, P, invP  = zca_whiten(data)
    end
...
-- if not specified, the transformation parameters are computed from data
function inv_zca_layer(data, means, P, invP)
    local auxdata
    if not means or not P or not invP then 
        auxdata, means, P, invP  = zca_whiten(data)
    end
...
function pca_whiten(data, means, P, invP)
    local auxdata = data:clone()
    local dims = data:size()
    local nsamples = dims[1]
    local n_dimensions = data:nElement() / nsamples
...
function pca_colour(data, means, P, invP)
    local auxdata = data:clone()
    local dims = data:size()
    local nsamples = dims[1]
    local n_dimensions = data:nElement() / nsamples
...
-- if not specified, the transformation parameters are computed from data
function pca_layer(data, means, P, invP)
    local auxdata
    if not means or not P or not invP then 
        auxdata, means, P, invP  = pca_whiten(data)
    end
    local n_dimensions = data:nElement() / data:size(1)
    local linear = nn.Linear(n_dimensions, n_dimensions)
    linear.weight:copy(P:t())
...
-- of its input using a precomputed (static) transformation matrix. 
-- if not specified, the transformation parameters are computed from data
function inv_pca_layer(data, means, P, invP)
    local auxdata
    if not means or not P or not invP then 
        auxdata, means, P, invP  = pca_whiten(data)
    end
