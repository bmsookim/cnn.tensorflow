...
local function createModel(opt)
   ...
   if opt.dataset == 'imagenet' then
      ...
      model:add(Convolution(3,64*factor,7,7,2,2,3,3))
      model:add(SBatchNorm(64*factor))
      model:add(ReLU(true))
      model:add(Max(3,3,2,2,1,1))
      model:add(layer(block, 64*factor, def[1]))
      model:add(layer(block, 128*factor, def[2], 2))
      model:add(layer(block, 256*factor, def[3], 2))
      model:add(layer(block, 512*factor, def[4], 2))
