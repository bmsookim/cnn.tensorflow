...
class BasicConvNet(object):
    def __init__(self, image_w=cf.w, image_h=cf.h, channels=cf.channels, num_classes=10):
        ...
        # GPU 1 input
        self._images1 = tf.placeholder(tf.float32, shape=[None, self._width, self._height, self._channels])
        # GPU 2 input
        self._images2 = tf.placeholder(tf.float32, shape=[None, self._width, self._height, self._channels])
        ...
        with tf.device('/gpu:0'):
            self._logits1 = self._inference(self._images1, self._keep_prob, self._is_train) # prediction
        
        # loss calculation & update (GPU2)
        with tf.device('/gpu:1'):
            tf.get_variable_scope().reuse_variables()
            self._logits2 = self._inference(self._images2, self._keep_prob, self._is_train) # prediction
    ...
    def score(self, X, y):
        ...
        for i in range(0, len(X), self._batch_size):
            ...
            total_loss += avg_loss * len(batch_labels)
